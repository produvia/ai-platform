...
from keras.layers import LeakyReLU, Concatenate, concatenate, UpSampling2D, Input
...
def incp_v3(x, nb_filters):
    ...
    b_1 = Conv2D(
        nb_filters, (1, 1), padding="same", kernel_initializer="glorot_normal"
    )(x)
    b_1 = BatchNormalization(axis=3)(b_1)
    b_1 = LeakyReLU()(b_1)
    b_1 = Conv2D(
        nb_filters, (3, 3), padding="same", kernel_initializer="glorot_normal"
    )(b_1)
    b_1 = BatchNormalization(axis=3)(b_1)
    b_1 = LeakyReLU()(b_1)
    b_1 = Conv2D(
...
def image_augmentation(imgs, masks, batch_size):
    ...
    image_generator = image_datagen.flow(
        imgs, batch_size=batch_size, shuffle=True, seed=seed
    )
...
@click.option(
    "--image_path",
    default="../finding-lungs-in-ct-data/2d_images/",
    help="Path to images folder",
)
@click.option(
    "--annotation_path",
    default="../finding-lungs-in-ct-data/2d_masks/",
    help="Path to annotations folder",
)
@click.option(
    "--weights_path", default="../weights/", help="Path to base model weights file"
)
...
def train(
    image_path, annotation_path, weights_path, log_dir, initial_lr, batch_size, seed
):
    ...
    tb = TensorBoard(
        log_dir=log_dir + "lungs_incp_unet_snapshot_mlflow",
        histogram_freq=0,
    ...
    callbacks = (
        [tb]
        + snapshot.get_callbacks(model_prefix=model_prefix)
        + [mlfow_log_callback()]
    )
    ...
    with mlflow.start_run() as run:
        ...
        pred = [
            model.predict(x.reshape((1, x.shape[0], x.shape[1], x.shape[2])))[0]
            for x in x_test
        ]
