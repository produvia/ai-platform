...
Run the command:
```bash
mlflow run -e generate . -P dataset=cats
```
...
```bash
mlflow run -e generate -P dataset=cats -P random_seed=777 -P nrow=2 -P ncol=5
```
This will generate 10 images at once.
### Image generation using checkpoints generated by this code
```bash
mlflow run -e genearte . \
...
```bash
mlflow run -e train . -P resume=False
```
